...
export const QRShare: React.FC<QRShareProps> = ({ value, strategId, onScan, status = 'idle' }) => {
  const { t } = useLanguage();
  ...
  const display = value || (strategId ? `STRATEG:${strategId}` : '');
  ...
  return (
    <div className="qr-share" style={{ textAlign: 'center', padding: '1rem' }}>
      <div style={{ background: '#fff', display: 'inline-block', padding: '0.5rem', borderRadius: '8px' }}>
        <QRCodeSVG value={display || ''} size={220} level="H" includeMargin />
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginTop: '0.5rem' }}>
        <input
          type="text"
          placeholder={t('messenger_scan') || 'Вставьте данные из QR'}
          value={inputValue}
      ...
      <button onClick={() => navigator.clipboard?.writeText(display || '')} style={{ marginTop: '0.5rem', background: 'transparent', border: 'none', color: 'var(--primary)', cursor: 'pointer' }}>
        Скопировать данные
      </button>
    </div>
  );
